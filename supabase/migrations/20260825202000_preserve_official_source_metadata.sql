-- Hivatalos növényvédőszer import v2: az ÚKSÚP/Nébih forrásból érkező extra mezők megőrzése.
...
declare
  ...
  v_product_metadata jsonb; v_use_metadata jsonb;
...
begin
  ...
  for r in select * from jsonb_array_elements(p_rows) loop
    ...
    v_product_metadata:=jsonb_strip_nulls(jsonb_build_object(
      'authorization_holder',nullif(trim(coalesce(r->>'authorization_holder',r->>'drzitel_autorizacie')),''),
      'authorization_period_raw',nullif(trim(coalesce(r->>'authorization_period_raw',r->>'doba_autorizacie_pripravku')),''),
      'sale_end_raw',nullif(trim(coalesce(r->>'sale_end_raw',r->>'datum_ukoncenia_predaja')),''),
      'use_end_raw',nullif(trim(coalesce(r->>'use_end_raw',r->>'datum_ukoncenia_spotreby')),''),
      'decision_status',nullif(trim(coalesce(r->>'decision_status',r->>'stav_rozhodnutia')),''),
      'product_type',nullif(trim(coalesce(r->>'product_type',r->>'typ_pripravku')),''),
      'user_type',nullif(trim(coalesce(r->>'user_type',r->>'typ_pouzivatela')),''),
      'parallel_trade',nullif(trim(coalesce(r->>'parallel_trade',r->>'paralelny_obchod')),''),
      'reference_product',nullif(trim(coalesce(r->>'reference_product',r->>'referencny_pripravok')),''),
      'formulation',nullif(trim(coalesce(r->>'formulation',r->>'formulacia')),''),
      'source_row',r
    ));
    ...
    if v_product_id is null then
      insert into public.plant_protection_products(
        country_code,name,authorization_number,function_type,regulatory_category,
        professional_use_only,prescription_required,approval_required,active,
        source_name,source_url,source_checked_at,source_snapshot_at,metadata
      ) values(
        ...
        false,
        case when p_country_code='HU' and upper(replace(coalesce(v_category,''),'.','')) in ('I','II') then true else false end,
        true,p_source_name,p_source_url,now(),now(),v_product_metadata
      ...
      update public.plant_protection_products set
        ...
        metadata=coalesce(metadata,'{}'::jsonb)||v_product_metadata
      where id=v_product_id;
    end if;

    v_crop:=nullif(trim(coalesce(r->>'crop',r->>'kultura',r->>'plodina',r->>'plodina_alebo_oblast_pouzitia')),'');
    v_target:=nullif(trim(coalesce(r->>'target',r->>'karosito',r->>'cel',r->>'skodlivy_organizmus_alebo_iny_ucel_pouzitia',r->>'skodlivy_cinitel')),'');
    ...
    if v_bbch_min is not null and v_bbch_max is not null and v_bbch_min > v_bbch_max then v_bbch_min:=null; v_bbch_max:=null; end if;
    ...
    v_use_metadata:=jsonb_strip_nulls(jsonb_build_object(
      'dose_raw',nullif(trim(r->>'dose_raw'),''),
      'phi_raw',nullif(trim(r->>'phi_raw'),''),
      'max_applications_season',nullif(trim(r->>'max_applications_season'),''),
      'seasonal_dose_raw',nullif(trim(r->>'seasonal_dose_raw'),''),
      'formulation',nullif(trim(coalesce(r->>'formulation',r->>'formulacia')),''),
      'tank_mix',nullif(trim(r->>'tank_mix'),''),
      'minor_use',nullif(trim(r->>'minor_use'),''),
      'source_row',r
    ));
    ...
    if v_crop is not null then
      ...
      if v_use_id is null then
        insert into public.plant_protection_uses(
          product_id,crop,target,dose_min,dose_max,dose_unit,application_method,phi_days,notes,metadata,
          ...
          v_product_id,v_crop,v_target,v_dose_min,v_dose_max,v_dose_unit,v_method,v_phi,p_source_name,v_use_metadata,
        ...
        update public.plant_protection_uses set
          ...
          metadata=coalesce(metadata,'{}'::jsonb)||v_use_metadata,
