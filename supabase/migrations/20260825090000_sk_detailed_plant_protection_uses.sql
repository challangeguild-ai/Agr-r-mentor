...
create index if not exists plant_protection_uses_detail_match_idx
  on public.plant_protection_uses (product_id, lower(crop), lower(coalesce(target,'')), dose_max, bbch_min, bbch_max);

do $$ begin
  alter table public.plant_protection_uses add constraint plant_protection_uses_bbch_min_check check (bbch_min is null or bbch_min between 0 and 99) not valid;
exception when duplicate_object then null; end $$;
do $$ begin
  alter table public.plant_protection_uses add constraint plant_protection_uses_bbch_max_check check (bbch_max is null or bbch_max between 0 and 99) not valid;
exception when duplicate_object then null; end $$;
do $$ begin
  alter table public.plant_protection_uses add constraint plant_protection_uses_application_count_check check (max_applications is null or max_applications > 0) not valid;
exception when duplicate_object then null; end $$;
do $$ begin
  alter table public.plant_protection_uses add constraint plant_protection_uses_water_volume_check check ((water_volume_min is null or water_volume_min >= 0) and (water_volume_max is null or water_volume_max >= 0)) not valid;
exception when duplicate_object then null; end $$;
...
begin
  ...
  for r in select * from jsonb_array_elements(p_rows) loop
    v_rows:=v_rows+1;
    v_product_id:=null; v_use_id:=null;
    ...
    where country_code=p_country_code
      and ((v_auth is not null and authorization_number=v_auth) or (v_auth is null and lower(trim(name))=lower(trim(v_name))))
    order by source_checked_at desc nulls last,id limit 1;
    ...
    if v_bbch_min is not null and v_bbch_max is not null and v_bbch_min>v_bbch_max then
      declare v_swap integer; begin v_swap:=v_bbch_min;v_bbch_min:=v_bbch_max;v_bbch_max:=v_swap; end;
    end if;

    if v_crop is not null then
      select id into v_use_id from public.plant_protection_uses
      where product_id=v_product_id
        and lower(trim(crop))=lower(trim(v_crop))
        and coalesce(lower(trim(target)),'')=coalesce(lower(trim(v_target)),'')
        and coalesce(dose_max,-1)=coalesce(v_dose_max,-1)
        and coalesce(lower(trim(dose_unit)),'')=coalesce(lower(trim(v_dose_unit)),'')
        and coalesce(lower(trim(application_method)),'')=coalesce(lower(trim(v_method)),'')
      ...
      if v_use_id is null then
        ...
        update public.plant_protection_uses set
          ...
          bbch_min=coalesce(v_bbch_min,bbch_min),bbch_max=coalesce(v_bbch_max,bbch_max),max_applications=coalesce(v_max_applications,max_applications),
          application_interval_days=coalesce(v_interval,application_interval_days),water_volume_min=coalesce(v_water_min,water_volume_min),
          water_volume_max=coalesce(v_water_max,water_volume_max),water_volume_unit=coalesce(v_water_unit,water_volume_unit),
          application_timing=coalesce(v_timing,application_timing),restrictions=coalesce(v_restrictions,restrictions),source_reference=coalesce(v_source_reference,source_reference)
        where id=v_use_id;
    ...
    if v_ingredient is not null and not exists(select 1 from public.plant_protection_ingredients where product_id=v_product_id and lower(trim(ingredient))=lower(trim(v_ingredient))) then
  ...
  return jsonb_build_object('batch_id',v_batch,'rows',v_rows,'inserted_products',v_products,'inserted_uses',v_uses,'updated_uses',v_updated_uses,'inserted_ingredients',v_ingredients);
end $$;
