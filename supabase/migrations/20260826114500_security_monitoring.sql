...
-- A raw security event cannot be forged by a normal authenticated client.
...
begin
  ...
  insert into public.security_events(event_type,severity,risk_score,ip_address,country_code,region,city,user_agent,request_path,method,subject_type,subject_id,detail,fingerprint)
  values(left(p_event_type,80),case when p_severity in ('info','low','medium','high','critical') then p_severity else 'info' end,greatest(0,least(coalesce(p_risk_score,0),100)),p_ip_address,left(p_country_code,8),left(p_region,120),left(p_city,120),left(p_user_agent,500),left(p_request_path,500),left(p_method,12),left(p_subject_type,80),left(p_subject_id,200),coalesce(p_detail,'{}'::jsonb),p_fingerprint)
  on conflict (fingerprint) where fingerprint is not null do update set created_at=excluded.created_at
  returning id into v_id;
  return v_id;
end $$;
revoke all on function public.record_security_event(text,text,integer,inet,text,text,text,text,text,text,text,text,jsonb,text) from public, anon, authenticated;
grant execute on function public.record_security_event(text,text,integer,inet,text,text,text,text,text,text,text,text,jsonb,text) to service_role;
