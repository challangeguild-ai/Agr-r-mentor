...
export default async function AdminPage(){
 ...
 const[{data:farmers},{data:farms},{data:fields},{data:reports},{data:tasks},{data:inspections},{data:visitEvents}]=await Promise.all([
  ...
  supabase.from("farmer_reports").select("id,title,status,created_at,field_id").order("created_at",{ascending:false}).limit(100),
  supabase.from("tasks").select("id,title,status,due_date,field_id,farm_id,priority").order("due_date",{ascending:true}).limit(300),
  supabase.from("inspections").select("id,field_id,inspected_at,condition,next_check_at,issue_status").order("inspected_at",{ascending:false}).limit(1000),
 ...
 const clientRows=(farmers??[]).map(f=>{const ownFarms=(farms??[]).filter(x=>x.owner_id===f.id),farmIds=ownFarms.map(x=>x.id),ownFields=(fields??[]).filter(x=>farmIds.includes(x.farm_id)),fieldIds=ownFields.map(x=>x.id),area=ownFields.reduce((s,x)=>s+Number(x.area_ha||0),0),crit=ownFields.filter(x=>latest.get(x.id)?.condition==="critical").length,att=ownFields.filter(x=>latest.get(x.id)?.condition==="attention").length,clientTasks=openTasks.filter(t=>farmIds.includes(t.farm_id)||fieldIds.includes(t.field_id)).length,clientReports=openReports.filter(r=>fieldIds.includes(r.field_id)).length,last=(inspections??[]).find(i=>fieldIds.includes(i.field_id)),score=crit*100+att*30+clientReports*20+clientTasks*10;return{f,ownFarms,ownFields,area,crit,att,clientTasks,clientReports,last,score}}).sort((a,b)=>b.score-a.score||String(a.f.full_name||"").localeCompare(String(b.f.full_name||""),"hu"));
 ...
 return <main className={styles.page}>
  <header className={styles.topbar}><div className={styles.welcome}><h1>Üdvözöljük, {me?.full_name||"Szaktanácsadó"}!</h1><p>Az ügyfelek állapota és a mai szakmai teendők egy helyen.</p></div><div className={styles.topActions}><NotificationBell/><div className={styles.year}>▣ {year}</div></div></header>
  ...
  <section className={styles.panel}><div className={styles.panelHead}><h2>Ügyfelek – hol van most dolgom?</h2><Link href="/admin/clients">Összes ügyfél →</Link></div><div className={styles.clientFocusGrid}>{clientRows.slice(0,6).map(r=><Link href={`/admin/clients/${r.f.id}`} className={styles.clientFocusCard} key={r.f.id}><div className={styles.clientFocusTop}><span className={styles.clientAvatar}>{(r.f.full_name||"G").slice(0,2).toUpperCase()}</span><div><strong>{r.f.full_name||"Névtelen ügyfél"}</strong><small>{r.ownFarms.length} gazdaság · {r.ownFields.length} tábla · {r.area.toLocaleString("hu-HU",{maximumFractionDigits:1})} ha</small></div><b className={r.crit?styles.clientCritical:r.att?styles.clientAttention:styles.clientGood}>{r.crit?`🔴 ${r.crit}`:r.att?`🟡 ${r.att}`:"🟢"}</b></div><div className={styles.clientFocusMeta}><span>{r.clientTasks} nyitott feladat</span><span>{r.clientReports} jelzés</span><span>Utolsó szemle: {d(r.last?.inspected_at)}</span></div></Link>)}</div></section>
