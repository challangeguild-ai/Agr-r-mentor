...
export async function syncUksupOfficialCatalog(){await advisorContext();const url="https://www.uksup.sk/storage/app/uploads/public/604/080/af4/604080af43e18263270214.csv";const r=await fetch(url,{cache:"no-store",headers:{"user-agent":"Agrar-Mentor/1.0 official-catalog-sync","accept":"text/csv,text/plain,*/*"}});if(!r.ok)throw new Error(`Az ÚKSÚP hivatalos CSV letöltése sikertelen: HTTP ${r.status}`);const text=await r.text();const rows=parseCsv(text);if(!rows.length)throw new Error("Az ÚKSÚP hivatalos CSV letöltődött, de a fejlécből nem sikerült készítményadatokat felismerni.");return runImport("SK","ÚKSÚP ISPOR – Prípravky na ochranu rastlín.csv",url,rows,`Automatikus hivatalos szinkron; feldolgozott sorok: ${rows.length}.`)}
