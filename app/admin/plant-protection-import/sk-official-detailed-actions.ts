...
export type SkOfficialDetailedSyncResult={ok:boolean;error?:string;rows?:number;inserted_products?:number;inserted_uses?:number;inserted_ingredients?:number;source_url?:string;sheets?:number};
...
function normalizeRow(original:Row){
 ...
 if(dose){const n=numbers(dose);if(n.length){row.dose_min=String(n.length>1?Math.min(...n):n[0]);row.dose_max=String(n.length>1?Math.max(...n):n[0])}if(!row.dose_unit)row.dose_unit=measureUnit(dose)}
 const water=String(row.water_raw||"");
 if(water){const n=numbers(water);if(n.length){row.water_volume_min=String(n.length>1?Math.min(...n):n[0]);row.water_volume_max=String(n.length>1?Math.max(...n):n[0])}row.water_volume_unit=measureUnit(water)||"l/ha"}
 const bbch=String(row.bbch_range||"");
 if(bbch){const n=numbers(bbch).map(Math.trunc);if(n.length){row.bbch_min=String(n[0]);row.bbch_max=String(n.length>1?n[n.length-1]:n[0])}}
 const phi=String(row.phi_raw||"").trim();
 if(phi){const n=numbers(phi);if(n.length)row.phi_days=String(Math.trunc(n[0]));else row.restrictions=appendText(row.restrictions||"",`Ochranná doba: ${phi}`)}
...
function sheetRows(sheet:XLSX.WorkSheet){
 ...
 let headerIndex=-1,header:string[]=[];
 for(let i=0;i<Math.min(matrix.length,60);i++){
   const keys=(matrix[i]||[]).map(v=>headerKey(String(v??"")));
   if(keys.includes("name")&&keys.includes("crop")){headerIndex=i;header=keys;break}
 }
 if(headerIndex<0)return[] as Row[];
 const out:Row[]=[];let carry:Row={};
 for(const cells of matrix.slice(headerIndex+1)){
   const row:Row={};header.forEach((h,i)=>{if(h)row[h]=String(cells?.[i]??"").trim()});
   for(const key of ["name","authorization_number","function_type","ingredient"]){if(row[key])carry[key]=row[key];else if(carry[key])row[key]=carry[key]}
   if(!row.name||!row.crop)continue;
   const n=normalizeRow(row);if(n.name&&n.crop)out.push(n);
 }
 return out;
}
...
function workbookRows(buffer:ArrayBuffer){
 ...
 const rows:Row[]=[];
 for(const name of wb.SheetNames){const sheet=wb.Sheets[name];if(sheet)rows.push(...sheetRows(sheet))}
 const dedup=new Map<string,Row>();
 for(const row of rows){const key=[row.name,row.authorization_number,row.crop,row.target,row.dose_max,row.bbch_min,row.bbch_max].map(v=>norm(v||"")).join("|");if(!dedup.has(key))dedup.set(key,row)}
 return{rows:[...dedup.values()],sheets:wb.SheetNames.length};
}
...
async function fetchBinary(url:string,timeout=30000){const c=new AbortController();const t=setTimeout(()=>c.abort(),timeout);try{const r=await fetch(url,{cache:"no-store",signal:c.signal,headers:{"user-agent":"Agrar-Mentor/1.0 UKSUP detailed sync","accept":"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel,*/*"}});if(!r.ok)throw new Error(`HTTP ${r.status}`);const b=await r.arrayBuffer();if(b.byteLength<10000)throw new Error("A letöltött XLSX túl kicsi.");return b}finally{clearTimeout(t)}}

async function discoverWorkbook(){
 const problems:string[]=[];
 for(const page of LIST_PAGES){
  try{
   const html=await fetchText(page);const anchors=[...html.matchAll(/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)];
   const candidates=anchors.map(m=>({href:m[1],text:stripHtml(m[2])})).map(x=>({...x,score:(norm(x.text).includes("rozsahom ich pouzitia")?100:0)+(norm(x.text).includes("autorizovanych a povolenych")?60:0)+(x.href.toLowerCase().includes("xlsx")?20:0)})).filter(x=>x.score>0).sort((a,b)=>b.score-a.score);
   for(const c of candidates){try{const url=new URL(c.href,page).toString();const buffer=await fetchBinary(url);const parsed=workbookRows(buffer);if(parsed.rows.length>=10)return{url,...parsed};problems.push(`${new URL(page).host}: XLSX ${parsed.rows.length} sor`)}catch(e){problems.push(`${new URL(page).host}: ${e instanceof Error?e.message:"XLSX hiba"}`)}}
   problems.push(`${new URL(page).host}: nem találtam megfelelő részletes XLSX hivatkozást`);
  }catch(e){problems.push(`${new URL(page).host}: ${e instanceof Error?e.message:"oldalhiba"}`)}
 }
 throw new Error(`A hivatalos ÚKSÚP részletes XLSX automatikus letöltése nem sikerült. ${problems.slice(-6).join("; ")}`);
}
...
export async function syncUksupOfficialDetailedUses():Promise<SkOfficialDetailedSyncResult>{
 try{
  const{supabase}=await advisorContext();const{url,rows,sheets}=await discoverWorkbook();
  const totals={rows:0,inserted_products:0,inserted_uses:0,inserted_ingredients:0};
  for(let i=0;i<rows.length;i+=400){const chunk=rows.slice(i,i+400);const{data,error}=await supabase.rpc("import_plant_protection_catalog",{p_country_code:"SK",p_source_name:"ÚKSÚP – hivatalos részletes felhasználási XLSX",p_source_url:url,p_rows:chunk,p_notes:`Automatikus részletes ÚKSÚP XLSX szinkron; ${rows.length} rekord, ${sheets} munkalap.`});if(error)throw new Error(error.message);const d:any=data||{};totals.rows+=Number(d.rows||chunk.length);totals.inserted_products+=Number(d.inserted_products||0);totals.inserted_uses+=Number(d.inserted_uses||0);totals.inserted_ingredients+=Number(d.inserted_ingredients||0)}
  revalidatePath("/admin/plant-protection-import");revalidatePath("/operations");return{ok:true,...totals,source_url:url,sheets};
 }catch(e){return{ok:false,error:e instanceof Error?e.message:"Az ÚKSÚP részletes automatikus szinkron ismeretlen hiba miatt megszakadt."}}
}
