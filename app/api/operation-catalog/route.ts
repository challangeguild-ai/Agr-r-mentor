...
export async function GET(request:NextRequest){
  ...
  const country=request.nextUrl.searchParams.get("country")||"HU",type=request.nextUrl.searchParams.get("type")||"spraying";
  ...
  if(type!=="spraying"&&type!=="plant_protection")return NextResponse.json({country,type,catalog:catalog??[],products:[],uses:[],source});

  const today=new Date().toISOString().slice(0,10);
  const{data:products,error:productError}=await supabase.from("plant_protection_products").select("id,name,authorization_number,function_type,valid_from,valid_until,source_name,source_checked_at").eq("country_code",country).eq("active",true).or(`valid_until.is.null,valid_until.gte.${today}`).order("name").limit(3000);
  if(productError)return NextResponse.json({error:productError.message},{status:500});
  const productIds=(products??[]).map(p=>p.id);
  if(!productIds.length)return NextResponse.json({country,type,catalog:catalog??[],products:[],uses:[],ingredients:[],source});
  ...
  return NextResponse.json({country,type,catalog:catalog??[],products:products??[],uses:uses??[],ingredients:ingredients??[],source});
}
