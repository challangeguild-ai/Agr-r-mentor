...
export default async function DashboardPage(){
  ...
  const[{data:farms},{data:tasks},{data:timeline},{data:documents},{data:lastInspection}]=await Promise.all([
    ...
    supabase.from("inspections").select("id,inspected_at,field_id,condition").order("inspected_at",{ascending:false}).limit(1).maybeSingle(),
  ]);
    ...
    <section className={styles.content}>
      <div className={styles.left}>
        <article className={styles.panel}>
          <div className={styles.panelHead}><h2>Tábláim térképen</h2><Link href="/fields">Teljes nézet ↗</Link></div>
          <div className={styles.map}><div className={styles.mapTools}><span>+</span><span>−</span><span>▱</span></div>{(fields??[]).slice(0,6).map((f,i)=><Link title={f.name} href={`/fields/${f.id}`} key={f.id} className={`${styles.fieldShape} ${styles[`s${i+1}`]}`}><b>{i+1}</b></Link>)}</div>
          {(fields?.length??0)>0?<><table className={styles.fieldTable}><thead><tr><th>Tábla neve</th><th>Növény</th><th>Terület</th><th>Év</th><th>Állapot</th></tr></thead><tbody>{(fields??[]).slice(0,5).map((f,i)=><tr key={f.id}><td><Link href={`/fields/${f.id}`}><span className={styles.number}>{i+1}</span><strong>{f.name}</strong></Link></td><td>{f.current_crop||"—"}</td><td>{f.area_ha?`${f.area_ha} ha`:"—"}</td><td>{f.crop_year||year}</td><td><span className={styles.status}>Nyilvántartva</span></td></tr>)}</tbody></table><div className={styles.mobileCards}>{(fields??[]).slice(0,5).map(f=><Link href={`/fields/${f.id}`} className={styles.mobileField} key={f.id}><span><strong>{f.name}</strong><br/>{f.current_crop||"Nincs kultúra"}</span><b>{f.area_ha?`${f.area_ha} ha`:"—"}</b></Link>)}</div></>:<div className={styles.empty}>Még nincs rögzített földtábla.</div>}
          <div className={styles.panelFooter}><Link href="/fields">Összes tábla megtekintése</Link></div>
        </article>
