...
export function NotificationBell(){
  ...
  useEffect(()=>{
    let alive=true;
    (async()=>{
      const{data,error:loadError}=await supabase.from("notifications").select("id,kind,title,message,href,read_at,created_at").order("created_at",{ascending:false}).limit(20);
      if(!alive)return;
  ...
  async function markRead(id:string){const now=new Date().toISOString();const{error:updateError}=await supabase.from("notifications").update({read_at:now}).eq("id",id);if(updateError){setError("Az értesítés nem jelölhető olvasottnak.");return}setItems(prev=>prev.map(x=>x.id===id?{...x,read_at:now}:x))}
  async function markAll(){const now=new Date().toISOString();const{error:updateError}=await supabase.from("notifications").update({read_at:now}).is("read_at",null);if(updateError){setError("Az értesítések nem jelölhetők olvasottnak.");return}setItems(prev=>prev.map(x=>({...x,read_at:x.read_at||now})))}
  return <div className="notification-wrap">
    ...
    {open&&<div className="notification-popover">
      ...
      {loading?<div className="notification-empty">Betöltés…</div>:items.length?<div className="notification-list">{items.map(item=><Link key={item.id} href={item.href||"#"} onClick={()=>markRead(item.id)} className={`notification-item ${item.read_at?"":"unread"}`}><div><strong>{item.title}</strong>{item.message&&<p>{item.message}</p>}<small>{new Date(item.created_at).toLocaleString("hu-HU")}</small></div></Link>)}</div>:!error&&<div className="notification-empty">Nincs új értesítés.</div>}
    </div>}
  </div>
}
